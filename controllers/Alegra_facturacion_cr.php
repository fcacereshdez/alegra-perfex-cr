...
private function get_product_type($item)
{
    ...
    $medical_service_keywords = [
        'consulta', 'examen', 'chequeo', 'revisión', 'diagnóstico',
        'cirugía', 'operación', 'procedimiento', 'terapia', 'sesión',
        'doctor', 'médico', 'especialista', 'evaluación'
    ];
    
    // Palabras clave para medicamentos
    $medicine_keywords = [
        'acetaminofen', 'paracetamol', 'ibuprofeno', 'aspirina', 
        'medicamento', 'medicina', 'fármaco', 'droga', 'pastilla',
        'cápsula', 'jarabe', 'inyección', 'ampolla', 'tableta'
    ];
...
private function save_iva_return_info($perfex_invoice_id, $alegra_invoice_id, $iva_return_items)
{
    ...
    /**
     * Función mejorada para obtener impuestos de Costa Rica
     */
...
private function get_payment_method($invoice)
{
    ...
    if (isset($invoice->allowed_payment_modes) && !empty($invoice->allowed_payment_modes)) {
        ...
        if (is_array($allowed_modes)) {
            foreach ($allowed_modes as $mode_id) {
                ...
                if (isset($payment_config['card_payment_methods']) && 
    ...
    if (isset($invoice->paymentmethod) && !empty($invoice->paymentmethod)) {
        ...
        if (strpos($payment_method, 'tarjeta') !== false || 
...
public function payment_methods_settings()
{
    ...
    /**
     * Obtiene el ID del impuesto en Alegra (debes configurar estos IDs según tu setup en Alegra)
     */
   /**
...
// En Alegra_facturacion_cr.php - Método settings() actualizado
...
public function settings()
{
    if ($this->input->post()) {
        ...
        // Procesar checkboxes que pueden no estar en POST si están desmarcados
        $checkbox_fields = [
        ...
        // Procesar array de métodos de pago para auto-transmisión
        ...
        if (empty($post_data['alegra_email'])) {
            set_alert('danger', 'El email de Alegra es requerido');
            redirect(admin_url('alegra_facturacion_cr/settings'));
            return;
        }
        
        // Validar que si está activada la auto-transmisión, tenga métodos configurados
        if ($post_data['auto_transmit_enabled'] == '1') {
            $payment_methods = json_decode($post_data['auto_transmit_payment_methods'], true);
            if (empty($payment_methods)) {
                set_alert('danger', 'Debe seleccionar al menos un método de pago para auto-transmisión');
                redirect(admin_url('alegra_facturacion_cr/settings'));
                return;
            }
        }
        
        // No guardar el token si está vacío (mantener el existente)
        if (empty($post_data['alegra_token'])) {
            unset($post_data['alegra_token']);
        }
        
        // Validar delay si está presente
        if (isset($post_data['auto_transmit_delay'])) {
            $delay = intval($post_data['auto_transmit_delay']);
            if ($delay < 0 || $delay > 60) {
                set_alert('danger', 'El retraso debe estar entre 0 y 60 minutos');
                redirect(admin_url('alegra_facturacion_cr/settings'));
                return;
            }
        }
        ...
        if ($success) {
            ...
            if (isset($post_data['auto_transmit_enabled']) && $post_data['auto_transmit_enabled'] == '1') {
                $payment_methods = json_decode($post_data['auto_transmit_payment_methods'], true);
                log_message('info', 'Alegra CR: Auto-transmisión activada por usuario ' . get_staff_user_id() . ' con métodos: ' . implode(',', $payment_methods));
            }
    ...
    // Obtener métodos de pago de Perfex para mostrar en la configuración
    $data['perfex_payment_modes'] = $this->alegra_cr_model->get_perfex_payment_modes();
    
    // Agregar estadísticas si están disponibles
    $data['stats'] = $this->alegra_cr_model->get_auto_transmit_stats();
...
public function get_auto_transmit_stats()
{
    ...
    /**
     * Vista para configuración de impuestos
     */
    public function tax_settings()
    {
        ...
        if ($this->input->post()) {
            $this->save_tax_settings();
            return;
        }

        $data['title'] = _l('alegra_cr_tax_settings');
        $data['tax_config'] = $this->alegra_cr_model->get_all_tax_config();
        $data['iva_returns'] = $this->alegra_cr_model->get_recent_iva_returns(50);

        $this->load->view('alegra_facturacion_cr/tax_settings', $data);
    }
     ...
     */
    /**
     * Versión mejorada de sync_taxes_from_alegra con mejor manejo de errores
     */
    ...
    // En Alegra_facturacion_cr.php - Agregar estos métodos de testing
